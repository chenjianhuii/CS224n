...
def naiveSoftmaxLossAndGradient(
    ...
    gradOutsideVecs = np.outer(centerWordVec, y_hat - y)
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    u[1:] *= -1
    temp = sigmoid(u @ centerWordVec)
    loss = - np.sum(np.log(temp))
    gradCenterVec = - u.T @ (1 - temp)
    gradOutsideVecs = np.zeros_like(outsideVectors)
    temp = 1 - temp
    temp[0] *= -1
    for i, idx in enumerate(indices):
        gradOutsideVecs[idx] += temp[i] * centerWordVec
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    outsideWordsIdx = [word2Ind[w] for w in outsideWords]
    centerWordVIdx = word2Ind[currentCenterWord]
    for idx in outsideWordsIdx:
        ls, gc, go = word2vecLossAndGradient(
            centerWordVectors[centerWordVIdx], idx,
            outsideVectors, dataset)
        loss += ls
        gradCenterVecs[centerWordVIdx] += gc
