...
if args.function == 'pretrain':
    ...
    if args.reading_params_path:
        ...
        hyperparameters = set_params(10)
    else:
        hyperparameters = set_params(75)
    train_txt = open(args.finetune_corpus_path, 'r').read()
    train_dataset = dataset.NameDataset(pretrain_dataset, train_txt)
    tconf = trainer.TrainerConfig(**hyperparameters)
    tconf.ckpt_path = args.writing_params_path
    trainerr = trainer.Trainer(model, train_dataset, None, tconf)
    trainerr.train()
    trainerr.save_checkpoint()
