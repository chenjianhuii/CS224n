...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        length = int(random.uniform(4, int(self.block_size*7/8)))
        truncated = document[:length]
        mask_len = int(random.uniform(1, int(length/2)))
        start = int(random.uniform(1, length-mask_len))
        prefix = truncated[:start]
        masked = truncated[start:start+mask_len]
        suffix = truncated[start+mask_len:]
        result = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked + self.MASK_CHAR
        result += self.PAD_CHAR * (self.block_size - len(result))
        result = [self.stoi[c] for c in result ]
        x = torch.tensor(result[:-1], dtype=torch.long)
        y = torch.tensor(result[1:], dtype=torch.long)
